package com.lnkj.privateshop.ui.near;

import android.support.annotation.NonNull;
import android.text.TextUtils;

import com.lnkj.privateshop.entity.ShopMerchandiseListBean;
import com.lnkj.privateshop.utils.LLog;
import com.lnkj.privateshop.utils.ToastUtil;

import org.json.JSONException;
import org.json.JSONObject;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import rx.android.schedulers.AndroidSchedulers;
import rx.functions.Action1;

import static com.lnkj.privateshop.utils.HttpUtil.meApi;

/**
 * Created by Administrator on 2017/8/3 0003.
 */

public class AddGoodsPresenter implements AddGoodsContract.Presenter {
    AddGoodsContract.View view;
    private String token;
    public AddGoodsPresenter(AddGoodsContract.View view) {
        this.view=view;

    }
    @Override
    public void getDataFromServer(List<ShopMerchandiseListBean.DataBean.GoodsListBean> lists,String context) {
        Map<String,Object> map = new HashMap<String, Object>();
        if (TextUtils.isEmpty(context)){
            ToastUtil.showToast("请输入内容");
            return;
        }else if (lists==null){
                ToastUtil.showToast("选择商品");
            return;
        }
     if (lists.size()==0){
         ToastUtil.showToast("选择商品");
         return;
     }

        map.put("token",token);
           StringBuffer sb = new StringBuffer();
           for (int i = 0; i < lists.size(); i++) {
               if (sb.length()!=0){
                   sb.append(",");
               }
               sb.append(lists.get(i).getGoods_id());
           }
         map.put("ids",sb.toString());

       map.put("content",context);
        view.showLoading();
       meApi.setShopCircle(map)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<String>() {
                    @Override
                    public void call(String data) {
                        view.hideLoading();
                        LLog.d("数据2", data);
                        JSONObject object = null;
                        try {
                            object = new JSONObject(data);
                             int status = object.getInt("status");
                            String info =object.getString("info");
                            if (status==1){
                            view.getOrderData();
                            }
                            ToastUtil.showToast(info);
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }


                    }
                }, new Action1<Throwable>() {
                    @Override
                    public void call(Throwable throwable) {
                        throwable.printStackTrace();
                        view.hideLoading();

                        LLog.d("数据错误11", throwable.toString() + "");
                    }
                });

    }


    @Override
    public void attachView(@NonNull AddGoodsContract.View view) {

    }

    @Override
    public void detachView() {

    }

    @Override
    public void getToken(String token) {
        this.token = token;
    }




}
