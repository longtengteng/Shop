...
import com.lnkj.privateshop.ui.seachgoods.SeachGoodsActivity;
...
//                    }
                ...
                if (content.equals("content")){
                    Intent intetn = new Intent(getActivity(), SeachGoodsActivity.class);
                    intetn.putExtra("id",mDatas.get(position).getId());
                    intetn.putExtra("name",mDatas.get(position).getName());
//                    System.out.println("ididid:"+mDatas.get(position).getId());
//                    System.out.println("ididid:"+mDatas.get(position).getTag());
//                    System.out.println("ididid:"+mDatas.get(position).getImgsrc());
//                    System.out.println("ididid:"+mDatas.get(position).getName());
//                    System.out.println("ididid:"+mDatas.get(position).getTitleName());
...
//                snackbar.show();
    ...
    private void initData() {
        ArrayList<GoodsCategoryBean.DataBean> rightList = getArguments().getParcelableArrayList("right");
        for (int i = 0; i < rightList.size(); i++) {
            RightBean head = new RightBean(rightList.get(i).getCat_name_mobile());
            //头部设置为true
            head.setTitle(true);
            head.setTitleName(rightList.get(i).getCat_name_mobile());
            head.setTag(String.valueOf(i));
            mDatas.add(head);
            ArrayList<GoodsCategoryBean.DataBean.ChildBean> categoryTwoArray = rightList.get(i).getChild();
            if (categoryTwoArray!=null){

            for (int j = 0; j < categoryTwoArray.size(); j++) {
                RightBean body = new RightBean(categoryTwoArray.get(j).getCat_name_mobile());
                body.setTag(String.valueOf(i));
                String name = categoryTwoArray.get(j).getCat_name_mobile();
                String img = categoryTwoArray.get(j).getImage();
                String id = categoryTwoArray.get(j).getCat_id();
                LLog.d("title",name);
