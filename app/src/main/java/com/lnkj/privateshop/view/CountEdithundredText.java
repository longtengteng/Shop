...
/**
 * 带有字数统计和限制的自定义EditText
 * 
 * @author WRJ
 * 
 */
public class CountEdithundredText extends android.support.v7.widget.AppCompatEditText {
	...
	private int numLength = 140;
	private String numStr = "0  /" + numLength;// 咳咳，没空格字数到百位了绘制的字数统计就撮到屏幕外面去了，有更好的方式的话请指教
	private int numSize = 22;// 字数统计的大小

	public CountEdithundredText(Context context) {
		this(context, null);
	}

	public CountEdithundredText(Context context, AttributeSet attrs) {
		super(context, attrs);
		edit = this;
		// 设置最多输入的字数
		edit.setFilters(new InputFilter[] { new InputFilter.LengthFilter(
				numLength) });
		mPaint = new Paint();
		int colorGray = getResources().getColor(R.color.text_64);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(numStr, editWidth - mBound.width() - 10, editHeight
				- mBound.height() - 5, mPaint);
	}
	...
	protected void onTextChanged(CharSequence text, int start,
		...
		/*
		 * 刷新该界面 invalidate()是用来刷新View的，必须是在UI线程中进行工作。
