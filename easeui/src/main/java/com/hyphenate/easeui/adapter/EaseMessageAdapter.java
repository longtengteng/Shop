...
import com.hyphenate.easeui.widget.chatrow.EaseChatRowGood;
import com.hyphenate.easeui.widget.chatrow.EaseChatRowGoodSend;
...
public class EaseMessageAdapter extends BaseAdapter {
    ...
    private static final int MESSAGE_TYPE_SENT_GOOD = 14;
    private static final int MESSAGE_TYPE_RECV_GOOD = 15;

    private static final int MESSAGE_TYPE_SENT = 16;
    private static final int MESSAGE_TYPE_RECV = 17;
    ...
    ChatGoodBean chatGoodBean;

    public ChatGoodBean getChatGoodBean() {
        return chatGoodBean;
    }

    public void setChatGoodBean(ChatGoodBean chatGoodBean) {
        this.chatGoodBean = chatGoodBean;
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == EMMessage.Type.TXT) {
            ...
            if ("sendGood".equals(message.getStringAttribute("type", ""))) {
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV : MESSAGE_TYPE_SENT;
            }
            if ("CommodityInfo".equals(message.getStringAttribute("type", ""))) {
                return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_GOOD : MESSAGE_TYPE_SENT_GOOD;
            }
    ...
    protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        switch (message.getType()) {
            case TXT:
                ...
                } else if ("sendGood".equals(message.getStringAttribute("type", ""))) {
                    chatRow = new EaseChatRowGoodSend(context, message, position, toChatUsername, this);
                } else if ("CommodityInfo".equals(message.getStringAttribute("type", ""))) {
                    chatRow = new EaseChatRowGood(context, message, position, this);
                } else {
    ...
    private void addGood(List<EMMessage> messageList) {
        if (chatGoodBean == null)
            return;
        EMMessage message = EMMessage.createTxtSendMessage("[产品详情]", EMClient.getInstance().getCurrentUser());
        message.setDirection(EMMessage.Direct.RECEIVE);
        message.setFrom("");
        message.setTo(EMClient.getInstance().getCurrentUser());
        message.setAttribute("type", "sendGood");
        message.setAttribute("commodityName", chatGoodBean.getCommodityName());
        message.setAttribute("commodityImage", chatGoodBean.getCommodityImage());
        message.setAttribute("commodityPrice", chatGoodBean.getCommodityPrice());
        message.setAttribute("commodityID", chatGoodBean.getCommodityID());
